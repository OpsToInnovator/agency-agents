...
|---|---|---|
| **Whoever wrote or edited a version can't approve it.** | `Service.approve` refuses the submitter and anyone who created or edited the draft since the last approval or discard (`Store.draft_authors`). Tests: `test_nobody_who_wrote_or_edited_the_draft_can_approve_it`, and the `refused` blocks in `tests/test_ads.py`. | It works per account, not per person. An owner adds members and receives their tokens, so one person holding two accounts could pass it. Never say "two people, guaranteed". |
| **A second maintainer approves the exact bytes, stamped with a SHA-256.** | Approval stores the SHA-256 of the stamped content. `skills history` and the approve output show it. | SkillCurrent manages the SKILL.md file only. Multi-file skills (scripts, reference files) aren't supported, so "exact bytes" means the SKILL.md. |
| **Early access first; everyone else when you say so.** | `skills release --channel canary`, `install --channel canary`, and the `skills history` line "canary = …, production = …". Tested in the `early` blocks. | The rollout is human-judged, with no traffic split, automatic metrics or automatic rollback. A machine joins early access by installing with `--channel canary`. Rolling back means moving the pointer back, and machines follow at their next sync. |
| **Machines running the hook report hand-edited and deleted copies; sync repairs missing and outdated ones and keeps edits.** | `Installer.status` and `sync` report `drift.modified`, `drift.missing` and `drift.outdated`. Sync repairs at the recorded path and keeps modified copies unless `--force`. `beta-report` counts it all. Tested in `test_beta_measurement.py` and the `noticed` blocks. | It covers only copies SkillCurrent installed, and only where the hook actually runs. Machines without the hook, and cloud agents, are not covered. Never write "every machine". |
...
| **Every terminal line here is real output.** | `tests/test_ads.py` builds each scenario and checks every shown line against what the CLI prints. A mutation check confirmed it fails on a changed line. | Some rows are omitted, and long lines are wrapped onto an indented continuation line. Digests are shortened with "…". |
