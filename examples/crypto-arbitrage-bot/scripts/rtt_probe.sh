...
#   A non-2xx answer (HTTP 451 geo-block, 403, 429) is not a round trip to the API: both times print as nan and
#   the code is kept so rollup.py can count the venue as blocked.
# Usage:  bash scripts/rtt_probe.sh >> rtt.log &        then  python3 scripts/rollup.py <logs> --rtt-log rtt.log
set -u
while true; do
  for u in "binance https://api.binance.com/api/v3/ping" "coinbase https://api.exchange.coinbase.com/time" "kraken https://api.kraken.com/0/public/Time"; do
    ...
    echo "$(date -u +%FT%TZ) $1 $(awk -v a="${out% *}" -v c="${out##* }" 'BEGIN{split(a,t," "); if (t[1]=="nan"||t[2]=="nan"||t[2]==0||c !~ /^2/) {print "warm_ms nan total_ms nan http " c} else {printf "warm_ms %.0f total_ms %.0f http %s", (t[2]-t[1])*1000, t[2]*1000, c}}')"
  done
  sleep 60
done
