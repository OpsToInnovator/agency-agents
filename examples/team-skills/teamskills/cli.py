"""Command-line interface. ``python -m teamskills --help``."""
...
from . import __version__, importer, skillfile
from .errors import Invalid, TeamSkillsError
from .installer import DEFAULT_TARGET, TARGETS, Installer
from .semver import BUMPS
from .service import ROLES, STATUSES, Service
from .session import LocalSession, RemoteSession, require_identity
from .store import Store

DEFAULT_DB = "~/.teamskills/teamskills.db"
...
def make_session(args):
    if args.server:
        ...
        return session
    if not args.team:
        raise Invalid("a team is required: pass --team or set TEAMSKILLS_TEAM")
    if not args.user:
        raise Invalid("an actor is required: pass --as or set TEAMSKILLS_USER")
...
def cmd_init(args, out):
    ...
    if not slug:
        raise Invalid("a team slug is required: teamskills init --team <slug> --owner <handle>")
    ...
    out.emit(
        ...
        lambda: (
            ...
            f"Next: export TEAMSKILLS_TEAM={result['team']['slug']} TEAMSKILLS_USER={result['owner']}"
        ),
    )
...
def _skill_line(s: dict) -> dict:
    return {**s, "version": s["latest_version"] or "-", "draft": "yes" if s["has_draft"] else ""}
...
def cmd_skills(args, out):
    ...
    if sub == "list" or sub is None:
        ...
        out.emit(
            rows,
            lambda: table(rows, [("slug", "SKILL"), ("status", "STATUS"), ("version", "VERSION"), ("draft", "DRAFT"), ("owner", "OWNER"), ("installs", "INSTALLS"), ("tags", "TAGS")]),
        )
    elif sub == "show":
        ...
        def human():
            lines = [
                ...
                f"  {s['description']}",
                f"  owner: {s['owner']}   latest: {s['latest_version'] or '-'}   installs: {s.get('installs', 0)}   tags: {', '.join(s['tags']) or '-'}",
            ]
            if s["has_draft"]:
        ...
        out.emit(s, lambda: f"Draft of {s['slug']} discarded; published version {s['latest_version']} stands.")
    elif sub == "delete":
        ...
        result = {"ok": not problems, "problems": problems, "warnings": warnings, "name": doc.name if doc else None}

        def human():
            lines = [f"{'OK' if result['ok'] else 'INVALID'}: {args.file}"]
        ...
        s = session.call("approve", slug=args.slug, comment=args.comment or "")
        out.emit(s, lambda: f"Published {s['slug']} {s['published']['version']} (authored by {s['published']['author']}).")
    elif sub == "reject":
...
def _installer(args) -> Installer:
    return Installer(make_session(args), project=Path(args.project) if args.project else None, custom_dir=args.dir)


def cmd_install(args, out):
    inst = _installer(args)
    results = [inst.install(slug, args.target, ref=args.ref) for slug in args.slugs]
...
def cmd_status(args, out):
    rows = _installer(args).status(args.target)
    out.emit(rows, lambda: table(rows, [("slug", "SKILL"), ("target", "TARGET"), ("installed", "INSTALLED"), ("latest", "LATEST"), ("state", "STATE"), ("path", "PATH")]))
    return 0 if all(r["state"] in ("current", "deprecated") for r in rows) else 2


def cmd_sync(args, out):
    rows = _installer(args).sync(args.target, force=args.force)
    out.emit(rows, lambda: table(rows, [("slug", "SKILL"), ("target", "TARGET"), ("action", "ACTION"), ("installed", "INSTALLED"), ("latest", "LATEST"), ("reason", "NOTE")]))
...
def cmd_dashboard(args, out):
    ...
    def human():
        ...
        lines = [
            ...
            f"  published {s['published']}   in review {s['in_review']}   drafts {s['draft']}   deprecated {s['deprecated']}",
        ]
...
def cmd_index(args, out):
    ...
    def human():
        ...
        lines.append(table(idx["skills"], [("slug", "SKILL"), ("version", "VERSION"), ("lifecycle", "LIFECYCLE"), ("published_at", "PUBLISHED"), ("content_hash", "SHA256")]))
        return "\n".join(lines)
...
def cmd_serve(args, out):
    ...
    mode = "no auth (identify with X-TeamSkills-Team / X-TeamSkills-User)" if args.no_auth else "member tokens"
    print(f"Team Skills {__version__} serving {Path(args.db).expanduser()} at http://{args.host}:{server.server_address[1]}/  [{mode}]")
    try:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="teamskills", description="Shared skill management for teams.")
    p.add_argument("--version", action="version", version=f"teamskills {__version__}")
    p.add_argument("--db", default=os.environ.get("TEAMSKILLS_DB", DEFAULT_DB), help="SQLite file (local mode) [TEAMSKILLS_DB]")
    p.add_argument("--team", default=os.environ.get("TEAMSKILLS_TEAM"), help="team slug [TEAMSKILLS_TEAM]")
    p.add_argument("--as", dest="user", default=os.environ.get("TEAMSKILLS_USER"), help="act as this member handle [TEAMSKILLS_USER]")
    p.add_argument("--server", default=os.environ.get("TEAMSKILLS_SERVER"), help="use a running `teamskills serve` instead of a local DB [TEAMSKILLS_SERVER]")
    p.add_argument("--token", default=os.environ.get("TEAMSKILLS_TOKEN"), help="member token for --server [TEAMSKILLS_TOKEN]")
    ...
    ks.add_parser("validate", help="check a SKILL.md offline").add_argument("file")
    ...
    ap = ks.add_parser("approve", help="approve the pending review and publish a version")
    ...
    i = sub.add_parser("install", help="install published skills into a tool's skills directory")
    i.add_argument("slugs", nargs="+")
    i.add_argument("--ref", help="'latest' (default), 'draft' or a version")
    add_target_args(i)
    ...
    st = sub.add_parser("status", help="compare installed copies with the catalog (exit 2 if anything needs attention)")
    add_target_args(st, with_target=False)
    st.set_defaults(fn=cmd_status)
    sy = sub.add_parser("sync", help="update outdated or missing installs")
    ...
    sv.add_argument("--no-auth", action="store_true", help="trust X-TeamSkills-* headers instead of tokens (demos only)")
